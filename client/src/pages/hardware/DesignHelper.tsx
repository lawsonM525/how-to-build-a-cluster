...
export const CoreVisual = ({ count, label, type = 'std' }: { count: number; label: string; type?: 'std' | 'big-little' }) => (
  <div className="flex flex-col items-center gap-2 p-3 bg-bg-secondary/5 rounded border border-border/50">
    <div className={`relative bg-primary/5 border border-primary/30 rounded p-1 flex flex-wrap content-center justify-center gap-1 transition-all hover:border-primary/60 hover:bg-primary/10 ${count > 8 ? 'w-20 h-20' : 'w-16 h-16'}`}>
